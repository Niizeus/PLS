...
## 9. Mise a jour terrain proche

Depuis la tranche LiDAR 2 m, le jeu garde `global.png` en base de relief pour couvrir toute la commune sans trou, puis charge a la demande la dalle 2 m autour du joueur. `TerrainLidar.tsx` affiche donc un fond global en 16 m et une dalle proche en maillage 4 m, issue des heightmaps IGN deja presentes dans `public/terrain`.

Au spawn, `World.tsx` precharge aussi la dalle fine de la cathedrale (purement cosmetique, voir §11) avant d'afficher le decor.
...
## 11. Décision : hauteur de jeu stable vs détail fin cosmétique (2026-07)

Les dalles fines 2 m se chargent **progressivement autour du joueur pendant la partie**, alors que quasiment tout le décor (routes, sols, pelouses, bâtiments, collisions...) ne construit sa géométrie **qu'une seule fois** au montage. Si `terrainHeight()` (`cityData.ts`) préférait la dalle fine dès qu'elle était chargée, la hauteur d'un même point pouvait changer en cours de partie selon le moment où chaque élément de décor avait été construit → routes/trottoirs/pelouses semblaient plonger sous le sol dans les zones découvertes plus tard (voir la note correspondante dans [`04-MONDE-BEAUVAIS.md`](04-MONDE-BEAUVAIS.md)).

Décision : `terrainHeight()` reste **garantie stable pour toute la partie** — elle ne lit que la carte globale 8 m (`global.png`), chargée une seule fois avant tout affichage, jamais les dalles fines. Le détail 2 m sert uniquement à faire "bomber" le maillage visuel affiché près du joueur au-dessus de cette base (jamais en dessous), dans `TerrainLidar.tsx`. Conséquence acceptée : les petites dépressions du relief fin (caniveaux, contre-pentes) ne se voient pas dans le sol affiché près du joueur — seuls les reliefs positifs (bosses, talus) ressortent. À revisiter si on veut le detail complet (creux compris) : il faudrait alors que TOUT le décor concerné se reconstruise dynamiquement quand une dalle proche finit de charger, ce qui est plus coûteux et plus complexe a maintenir correctement partout.
