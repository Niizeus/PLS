  ...
  return (
    <div style={appScroll}>
      ...
      {nearby.map(({ marker, distance }) => {
        ...
        return (
          <div key={marker.id} style={{ ...card, display: 'flex', alignItems: 'center', gap: 9 }}>
      ...
      {waypoints.length > 0 && (
        <>
          ...
          {waypoints.map((wp) => (
            <div key={wp.id} style={{ ...card, display: 'flex', alignItems: 'center', gap: 9 }}>
      ...
      <div style={{ font: `10px ${PHONE.font}`, color: PHONE.muted, lineHeight: 1.45 }}>
        Les points de passage se posent sur la grande carte (touche M). Le GPS ne fait que les
        afficher.
      </div>
    </div>
  )
